...
public class SelfDiagnosisService {
	...
	//자가진단표 생성
	@Transactional
	public void createDiagnosis(SelfDiagnosisRequest request, Long userId) {
	...
	//자가진단표 수정
	@Transactional
	public void updateDiagnosis(Long diagnosisId, Long userId, SelfDiagnosisRequest request) {
		SelfDiagnosis diagnosis = selfDiagnosisRepository.findByIdAndUserId(diagnosisId, userId)
			.orElseThrow(() -> new OuchException(DiagnosisErrorCode.DIAGNOSIS_NOT_FOUND));
